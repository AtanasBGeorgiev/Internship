...
router.post('/RegisterUser', async (req: Request, res: Response, next: NextFunction) => {
  console.log('POST /RegisterUser accepted!');
  try {
    ...
    const user = new User({ role: "admin", ...userData, password: hashedPassword });// Creates a new document (user) with these data, based on the mongoose model User
...
router.post('/RegisterAdmin', async (req: Request, res: Response, next: NextFunction) => {
  console.log('POST /RegisterAdmin accepted!');
  try {
    const { username, password, egn, phone } = req.body;
