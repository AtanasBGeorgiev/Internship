...
import type { Account, Payment, Card, Liability, Transaction, Credit, Deposit, Currency } from '../Components/ModelTypes';
                    ...
                    tableHead={
                        <>
                            <TableData type="th" text={t("Наименование")} alignment="left" checkbox={
                                <GroupCheckbox allSelected={allSelectedLiabilities} type="parent" toggleSelectAll={toggleSelectAllLiabilities} />
                            } />
                            <TableData type="th" text={t("Към дата")} />
                            <TableData type="th" text={t("Автом. плащане")} />
                            <TableData type="th" text={t("Сума")} />
                            <TableData type="th" text={""} />
                        </>
                    }
                    items={liabilities}
                    tableData={(liability) => (
                        <>
                            <TableData text={`${liability.name}`} type="bill" iconType={liability.feeType} alignment="left"
                                checkbox={
                                    <GroupCheckbox isSelected={selectedLiabilities.includes(liability.id)}
                                        onToggle={() => toggleLiability(liability.id)} />
                                } />
                    ...
                    tableHead={
                        <>
                            <TableData type="th" text={t("Вид кредит")} alignment="left" checkbox={
                                <GroupCheckbox allSelected={allSelectedCredits} type="parent" toggleSelectAll={toggleSelectAllCredits} />
                            } />
                            <TableData type="th" text={t("Валута")} />
                        ...
                        <>
                            <TableData text={`${credit.name}`} type="accountInfo" icon={<FaHandHoldingDollar className="text-blue-800 text-3xl transform scale-x-[-1]" />}
                                amount={`${credit.amount}`} alignment="left" checkbox={
                                    <GroupCheckbox isSelected={selectedCredits.includes(credit.id)}
                                        onToggle={() => toggleCredit(credit.id)} />
                                } />
